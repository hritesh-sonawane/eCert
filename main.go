...
func main() {
	name := flag.String("name", "Hritesh Sonawane", "the name of the person who completed the course")
	flag.Parse()
	...
	_, lineHt = fpdf.GetFontSize()
	fpdf.WriteAligned(0, lineHt*1.5, "What do you call a turtle that surfs the dark web? A TORtoise", gofpdf.AlignCenter)
	pdf.Move(0, lineHt*4.5)

	fpdf.ImageOptions("images/acc.jpeg", w/2.0-50.0, pdf.y, 100.0, 0, false, gofpdf.ImageOptions{
		ReadDpi: true,
	}, 0, "")
	...
	pdf.Text("Instructor - Hritesh Sonawane")
	...
	sig, err := gofpdf.SVGBasicFileParse("images/sign.svg")
	if err != nil {
		panic(err)
	}
	...
	// Grid
	// drawGrid(fpdf)
	err = fpdf.OutputFileAndClose("test_cert.pdf")
	if err != nil {
		panic(err)
	}
}
